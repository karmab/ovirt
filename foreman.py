...
def foremando(url, actiontype=None, postdata=None, user=None, password=None):
    ...
    if actiontype == 'POST':
        r = requests.post(url, verify=False, headers=headers, auth=(
            user, password), data=json.dumps(postdata))
    elif actiontype == 'DELETE':
        r = requests.delete(url, verify=False, headers=headers,
                            auth=(user, password), data=postdata)
    elif actiontype == 'PUT':
...
def foremangetid(protocol, host, port, user, password, searchtype, searchname):
    if searchtype == 'puppet':
        url = "%s://%s:%s/api/v2/smart_proxies?type=%s" % (
            protocol, host, port, searchtype)
        result = foremando(url)
        return result[0]['smart_proxy']['id']
    else:
        url = "%s://%s:%s/api/v2/%s/%s" % (protocol,
                                           host, port, searchtype, searchname)
...
class Foreman:
    ...
    def delete(self, name, dns=None):
        ...
        result = foremando(url=url, actiontype='DELETE',
    ...
    def create(self, name, dns, ip, mac=None, operatingsystem=None, environment=None, arch="x86_64", puppet=None, ptable=None, powerup=None, memory=None, core=None, compute=None, profile=None, hostgroup=None, build=False, location=None, organization=None):
        ...
        if operatingsystem:
            osid = foremangetid(protocol, host, port, user,
                                password, 'operatingsystems', operatingsystem)
            postdata['host']['operatingsystem_id'] = osid
        if not hostgroup:
            if not environment:
                environment = "production"
            environment = environment.encode('ascii')
            envid = foremangetid(protocol, host, port, user,
                                 password, 'environments', environment)
            postdata['host']['environment_id'] = envid
        postdata['host']['build'] = build
        if arch:
            archid = foremangetid(protocol, host, port,
                                  user, password, 'architectures', arch)
            postdata['host']['architecture_id'] = archid
        if puppet:
            puppetid = foremangetid(
                protocol, host, port, user, password, 'puppet', puppet)
        ...
        if compute:
            computeid = foremangetid(
                protocol, host, port, user, password, 'compute_resources', compute)
            postdata['host']['compute_resource_id'] = computeid
        if profile:
            profileid = foremangetid(
                protocol, host, port, user, password, 'compute_profiles', profile)
            postdata['host']['compute_profile_id'] = profileid
        if hostgroup:
            hostgroupid = foremangetid(
                protocol, host, port, user, password, 'hostgroups', hostgroup)
            postdata['host']['hostgroup_id'] = hostgroupid
        if ptable:
            ptableid = foremangetid(
                protocol, host, port, user, password, 'ptables', ptable)
            postdata['host']['ptable_id'] = ptableid
        if location:
            locationid = foremangetid(
                protocol, host, port, user, password, 'locations', location)
            postdata['host']['location_id'] = locationid
        if organization:
            postdata['host']['organization_id'] = orgid(
                protocol, host, port, user, password, organization)
        result = foremando(url=url, actiontype="POST",
                           postdata=postdata, user=user, password=password)
    ...
    def addclasses(self, name, dns, classes):
        ...
        for classe in classes:
            classid = foremangetid(protocol, host, port,
                                   user, password, 'puppetclasses', classe)
            url = "%s://%s:%s/api/v2/hosts/%s.%s/puppetclass_ids" % (
                protocol, host, port, name, dns)
            postdata = {'puppetclass_id': classid}
            foremando(url=url, actiontype="POST", postdata=postdata,
                      user=user, password=password)
            print "class %s added to %s.%s" % (classe, name, dns)

    def hostgroups(self, environment):
        host, port, user, password, protocol = self.host, self.port, self.user, self.password, self.protocol
        url = "%s://%s:%s/api/v2/hostgroups?per_page=%s" % (
            protocol, host, port, perpage)
        res = foremando(url=url, user=user, password=password)
        results = []
        for r in res['results']:
    ...
    def classes(self, environment):
        host, port, user, password, protocol = self.host, self.port, self.user, self.password, self.protocol
        url = "%s://%s:%s/api/v2/puppetclasses?search=environment+=+%s&per_page=%s" % (
            protocol, host, port, environment, perpage)
        res = foremando(url=url, user=user, password=password)
        results = []
        res = res['results']
        for module in res:
    ...
    def classinfo(self, name):
        host, port, user, password, protocol = self.host, self.port, self.user, self.password, self.protocol
        url = "%s://%s:%s/api/v2/puppetclasses/%s" % (
        ...
        for parameter in parameters:
            ...
            parameterurl = "%s://%s:%s/api/v2/smart_class_parameters/%s-%s" % (
                protocol, host, port, parameterid, parametername)
    ...
    def override(self, name, parameter, parameterid=None):
        ...
        if not parameterid:
            ...
            if parameterid is None:
                ...
                return False
            parameterurl = "%s://%s:%s/api/v2/smart_class_parameters/%s-%s" % (
                protocol, host, port, parameterid, parameter)
            postdata = {}
            postdata["smart_class_parameter"] = {"override": True}
            postdata = simplejson.dumps(postdata)
            res = foremando(url=parameterurl, actiontype="PUT",
                            postdata=postdata, user=user, password=password)
    ...
    def addparameters(self, name, dns, parameters):
        ...
        parametersid = {}
        parametersurl = "%s://%s:%s/api/v2/smart_class_parameters?per_page=10000" % (
            protocol, host, port)
        ...
        for parameter in parameters:
            try:
                ...
                continue
            if parameter not in parametersid.keys():
            ...
            parameterurl = "%s://%s:%s/api/v2/smart_class_parameters/%s-%s" % (
                protocol, host, port, parameterid, parameter)
            res = foremando(url=parameterurl, user=user, password=password)
            override, override_values, override_values_count, parameter_type = res['override'], res[
                'override_values'], res['override_values_count'], res['parameter_type']
            if not override:
                postdata = {}
                postdata["smart_class_parameter"] = {"override": True}
                postdata = simplejson.dumps(postdata)
            ...
            if len(override_values) > 0:
                for o in override_values:
                    ...
                    if match[0] == 'fqdn' and match[1] == "%s.%s" % (name, dns):
                        ...
                        break
            if overrideid == 0:
                postdata = {"override_value": {
                    "match": "fqdn=%s.%s" % (name, dns)}}
                if parameter_type == 'string':
                    postdata["override_value"]["value"] = value
                elif parameter_type == 'array':
                    value = value.split(',')
                    postdata["override_value"]["value"] = value
                overrideurl = "%s://%s:%s/api/v2/smart_class_parameters/%s/override_values" % (
                    protocol, host, port, parameter)
                res = foremando(url=overrideurl, actiontype="POST",
                                postdata=postdata, user=user, password=password)
                ...
                if parameter_type == 'string':
                    ...
                    postdata["override_value"]["value"] = value
                postdata = simplejson.dumps(postdata)
                overrideurl = "%s://%s:%s/api/v2/smart_class_parameters/%s/override_values/%s" % (
                    protocol, host, port, parameter, overrideid)
                res = foremando(url=overrideurl, actiontype="PUT",
                                postdata=postdata, user=user, password=password)
    ...
    def start(self, name, dns=None):
        ...
        if '.' in name:
            ...
            name = name.split('.')[0]
        url = "%s://%s:%s/api/v2/hosts/%s.%s/power" % (
            protocol, host, port, name, dns)
        postdata = {}
        postdata['power_action'] = 'start'
        postdata = simplejson.dumps(postdata)
        res = foremando(url=url, actiontype="PUT",
                        postdata=postdata, user=user, password=password)
...
if __name__ == '__main__':
    ...
    creationgroup.add_option('-H', '--host', dest='host',
                             type='string', help='foreman host')
    creationgroup.add_option('-P', '--port', dest='port',
                             type='string', default='443', help='foreman port')
    creationgroup.add_option('-u', '--user', dest='user',
                             type='string', default='admin', help='foreman port')
    creationgroup.add_option('-p', '--password', dest='password',
                             type='string', default='changeme', help='foreman password')
    creationgroup.add_option('-n', '--new', action="store_true", help='new')
    creationgroup.add_option('-d', '--dns', dest='dns',
                             type='string', help='dns')
    creationgroup.add_option('-i', '--ip', dest='ip', type='string', help='ip')
    creationgroup.add_option('-k', '--kill', dest="kill",
                             action="store_true", help="Kill machine")
    creationgroup.add_option(
        '-l', '--location', dest='location', type='string', help='location')
    creationgroup.add_option('-m', '--mac', dest='mac',
                             type='string', help='mac')
    creationgroup.add_option(
        '-O', '--organization', dest='organization', type='string', help='Organization')
    creationgroup.add_option(
        '-X', '--hostgroup', dest='hostgroup', type='string', help='hostgroup')
    creationgroup.add_option(
        '-b', '--build', action="store_true", help='build')
    creationgroup.add_option(
        '-c', '--compute', dest='compute', type='string', help='compute')
    creationgroup.add_option('-Z', '--profile', dest='profile',
                             type='string', default='1-Small', help='profile')
    parser.add_option_group(creationgroup)
    listinggroup = optparse.OptionGroup(parser, "Listing options")
    listinggroup.add_option("-a", "--archs", dest="listarchs",
                            action="store_true", help="List archs")
    listinggroup.add_option("--domains", dest="listdomains",
                            action="store_true", help="List domains")
    listinggroup.add_option("--environments", dest="listenvironments",
                            action="store_true", help="List environments")
    listinggroup.add_option("--hostgroups", dest="listhostgroups",
                            action="store_true", help="List hostgroups")
    listinggroup.add_option('-V', '--listvms', dest="listvms",
                            action="store_true", help="List Machines")
    listinggroup.add_option("--oses", dest="listos",
                            action="store_true", help="List os")
    listinggroup.add_option("--puppets", dest="listpuppets",
                            action="store_true", help="List puppets")
    listinggroup.add_option("-L", "--clients", dest="listclients",
                            action="store_true", help="list available clients")
    listinggroup.add_option("-R", "--computes", dest="listcomputes",
                            action="store_true", help="List compute resources")
    listinggroup.add_option("-9", "--switchclient", dest="switchclient",
    ...
    if not client:
        try:
            ...
            os._exit(1)
    try:
        if not host and 'host' in foremans[client].keys():
            host = foremans[client]["host"]
        if not port and 'port' in foremans[client].keys():
            port = foremans[client]['port']
        if not user and 'user' in foremans[client].keys():
            user = foremans[client]['user']
        if not password and 'password' in foremans[client].keys():
            password = foremans[client]['password']
        if not mac and 'mac' in foremans[client].keys():
        ...
        if not organization and 'organization' in foremans[client].keys():
            organization = foremans[client]['organization']
        if not location and 'location' in foremans[client].keys():
    ...
    name = args[0]
    if new:
        f.create(name=name, dns=dns, ip=ip, mac=mac, hostgroup=hostgroup, compute=compute,
                 profile=profile, build=build, location=location, organization=organization)
        sys.exit(0)
    if kill:
